...
float driveMotor1;
float driveMotor2;
float forwardSpeed = 118;
float reverseSpeed = 80;
float forwardSpeedTemp = forwardSpeed;
float reverseSpeedTemp = reverseSpeed;
float forwardSpeedFast = 128;
float reverseSpeedFast = 70;
float forwardSpeedSlow = 118;
float reverseSpeedSlow = 80;
float xvalueDelay = 0;
float yvalueDelay = 0;
float rumble = 0;
float leftRumble = 0;
...
void loop() {
  if (PS4.isConnected()) {
    ...
    // L2 Weapon Control
    if(PS4.R2Value()>10){
      value = map(PS4.R2Value(), 10, 255, 30, 45);
      rightRumble = PS4.R2Value();
    }
    if(PS4.L2Value()>10){
      value = map(PS4.L2Value(), 10, 255, 30, 37);
    ...
    */
    ...
    /*
      ...
      driveMotor2 = map(PS4.LStickY(), -128, 127, 142, 50);
...
void onEvent() {
  ...
  if (PS4.event.button_down.cross) {
    if (slowSpeed == true) {
      if (forwardSpeed == forwardSpeedSlow) {
        forwardSpeed = reverseSpeedSlow;
        reverseSpeed = forwardSpeedSlow;
      } else {
        forwardSpeed = forwardSpeedSlow;
        reverseSpeed = reverseSpeedSlow;
      }
    } else {
      if (forwardSpeed == forwardSpeedFast) {
        forwardSpeed = reverseSpeedFast;
        reverseSpeed = forwardSpeedFast;
      } else {
        forwardSpeed = forwardSpeedFast;
        reverseSpeed = reverseSpeedFast;
      }
    }
